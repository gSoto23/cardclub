...
export default function TournamentsAdmin() {
  ...
  const deleteRegistration = async (regId: number) => {
    if (!confirm("¿Seguro que deseas eliminar a este usuario del torneo? Esto también eliminará su pago y ventas asociadas en el sistema (sin reembolso automático).")) return;
    
    const token = localStorage.getItem("auth_token");
    const toastId = toast.loading('Eliminando inscripción...');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tournaments/registrations/${regId}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (res.ok) {
        ...
        const errorData = await res.json();
        toast.error(errorData.detail || 'Error al eliminar la inscripción', { id: toastId });
      }
    } catch (err) {
      toast.error('Error de conexión', { id: toastId });
    }
